...
## USAGE: This script is meant to be used after running family_based_pca.R

# Rscript PC_filter_related.R --data_id abcd1 --cutoff_file cutoff.txt
...
if ( is.null(opt$pca_file      ) ) { opt$pca_file      = paste0(opt$data_id,'.related_pca.eigenvec')     }
...
if ( opt$verbose ) { 
    ...
    cat(paste0('pca file is = ',opt$pca_file),'\n')
    cat(paste0('cutoffs = ',cutoffs),'\n')
    cat(paste0('cutoff file = ',opt$cutoff_file),'\n')
}
...
if (cutoffs == TRUE) { 
  ...
  fam <- read.table(paste0(opt$pcaer_dir,'/',opt$data_id,'.mepr.fam'),h=F,stringsAsFactors=F)
  ...
  ## read in PC file
  pc <- read.table(opt$pca_file,h=T,stringsAsFactors=F)
  ...
  cutoff_pc <- paste0('P',cf2$PC[i]) ## NOTE that C1 turns into PC1 here
